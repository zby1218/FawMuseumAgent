...
dispatch 是一个明确知道要调什么的确定性节点（从 `skill_sequence[current_step]` 读取），不需要让 LLM 动态决策，因此 **不使用 LangChain ToolNode / ReAct 模式**，调用链为：

```
dispatch 节点（普通 Python 函数）
    ↓
ros_tools.py（普通封装层，做隔离和名称映射）
    ↓
ros2_bridge.py（来自 fawbot_skills，三种模式）
    ↓
ROS2 Action / WebSocket rosbridge / Mock
```

`ros_tools.py` 通过 `get_ros2_bridge()` 单例获取 bridge，不自己 new 实例（bridge 初始化涉及 rclpy.init 和 spin 线程，只能做一次）：

```python
# tools/ros_tools.py
from fawbot_skills.robot_interface.ros2_bridge import get_ros2_bridge

SUPPORTED_SKILLS = {"navigate", "dance", "grasp", "place"}

async def execute_skill(skill: str, params: dict) -> dict:
    if skill not in SUPPORTED_SKILLS:
        return {
            "success": False,
            "error": f"skill '{skill}' 不存在，可用: {sorted(SUPPORTED_SKILLS)}"
        }
    bridge = get_ros2_bridge()  # 单例，启动时已 connect
    match skill:
        case "navigate": return await bridge.navigate_to_location(**params)
        case "dance":    return await bridge.play_action(**params)   # 名称映射
        case "grasp":    return await bridge.grasp(**params)
        case "place":    return await bridge.release(**params)       # 名称映射
```

`get_ros2_bridge()` 通过 `ROS2_BRIDGE_MODE` 环境变量自动选择模式：`direct`（生产）、`rosbridge`（远程调试）、`mock`（测试，无需真实 ROS2 环境）。

#### ROS 回调机制

ROS2 Action 本身是异步的（发 Goal → 等 Result）。dispatch 调用 `execute_skill` 时，在后台 task 里等待 Action 完成，完成后触发 `Command(resume=result)` 恢复 invoke 1：

```python
# callback/ros_callback.py
async def ros_action_and_resume(graph, config, skill: str, params: dict):
    result = await execute_skill(skill, params)
    await graph.ainvoke(
        Command(resume={"success": result["success"], "message": result.get("message", "")}),
        config
    )
```

不需要单独的 HTTP callback 端点，也不需要订阅 ROS topic。
...
- [x] ROS 回调的具体触发方式：复用 `fawbot_skills/ros2_bridge.py`，后台 task 等 ROS2 Action 完成后 `Command(resume=result)`，不需要 HTTP endpoint 或独立 topic
